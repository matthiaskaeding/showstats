...
Input is polars-native; non-polars input (pandas, pyarrow, etc.) is
converted via `narwhals` in `_check_input_maybe_try_transform`, with a
`pl.DataFrame(...)` fallback for plain Python literals (list/dict) that
narwhals doesn't recognize as native dataframes.
