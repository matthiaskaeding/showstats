...
def test_table_one_groups_statistics_by_another_column(capsys):
    frame = pl.DataFrame(
        {
            "age": [1.0, 3.0, 10.0, 14.0],
            "outcome": ["yes", "no", "yes", "yes"],
            "arm": ["control", "control", "treated", "treated"],
        }
    )

    table_one(frame, group="arm")

    output = capsys.readouterr().out
    assert "arm = control (N=2)" in output
    assert "arm = treated (N=2)" in output
    assert "age (mean (SD))" in output
    assert "2.0 (1.41)" in output
    assert "12.0 (2.83)" in output
    assert "outcome = yes (%)" in output
    assert "arm = control (%)" not in output


def test_table_one_grouping_accepts_lazy_input(capsys):
    frame = pl.DataFrame(
        {
            "age": [1.0, 3.0, 10.0, 14.0],
            "arm": ["control", "control", "treated", None],
        }
    )

    table_one(frame.lazy(), group="arm")
    lazy_output = capsys.readouterr().out
    table_one(frame, group="arm")

    assert lazy_output == capsys.readouterr().out
    assert "arm = Missing (N=1)" in lazy_output


def test_table_one_rejects_an_unknown_group_column():
    with pytest.raises(ValueError, match="group column 'unknown' not found"):
        table_one(MIXED, group="unknown")


def test_table_one_rejects_a_nonstring_group_column():
    with pytest.raises(TypeError, match="group must be a column name"):
        table_one(MIXED, group=1)
